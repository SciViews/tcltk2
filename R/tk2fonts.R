...
tk2font.get <- function(font, what = c("family", "size", "bold", "italic")) {
  ...
  res <- list()
  if (length(fontspec) != 12)
    return(res)  # There is a problem here!
  if ("family" %in% what)
    res$family <- fontspec[2]
  if ("size" %in% what)
    res$size <- as.numeric(fontspec[4])
  if ("bold" %in% what)
    res$bold <- (fontspec[6] == "bold")
  if ("italic" %in% what)
    res$italic <- (fontspec[8] == "italic")
  if ("underline" %in% what)
    res$underline <- (fontspec[10] == "1")
  if ("overstrike" %in% what)
    res$overstrike <- (fontspec[12] == "1")
  res
}
...
tk2font.set  <- function(font, settings) {
  ...
  if (is.character(settings)) {
    ...
    for (i in 1:l) {
      ...
      .Tcl(paste("catch {font configure ", font[i], " ", settings[i], "}", sep = ""))
    }
    ...
    for (i in 1:l) {
      ...
      if (!is.null(fntl$family)) {
        ...
        if (length(fntfamily) > 1) {
          ...
          if (any(fntexists)) {
            fntfamily <- fntfamily[fntexists][1]
          } else {
      ...
      if (!is.null(fntl$size))
        fnt <- paste(fnt, "-size", fntl$size)
      ...
      if (!is.null(fntl$underline))
        fnt <- paste(fnt, "-underline",
          as.numeric(fntl$underline == TRUE))
      if (!is.null(fntl$overstrike))
        fnt <- paste(fnt, "-overstrike",
