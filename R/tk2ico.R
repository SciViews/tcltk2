...
`tk2ico.pos<-` <- function(icon, value) {
  ...
  #  sep = "")))
  #if (res != "0")
  #  stop("Error while changing default position of the icon!")
  #icon
}
...
tk2ico.set <- function(win, icon, pos = NULL, type = c("all", "small", "big")) {
  ...
  if (!missing(pos))
    warning("'pos' argument deprecated from tcltk2 1.2-2")
  if (!missing(type))
    warning("'type' argument deprecated from tcltk2 1.2-2")
...
tk2ico.taskbar.delete <- function(icon) {
  ...
  #res <- tclvalue(.Tcl(paste("catch {winico taskbar delete ", icon, "}", sep = "")))
  #(res == "0")  # return "0" if OK, "1" otherwise
}
...
tk2ico.taskbar.modify <- function(icon, pos = NULL, text = NULL) {
  ...
  #(tclvalue(.Tcl(cmd)) == "0")   # "0" if OK, "1" otherwise
}
...
`tk2ico.text<-` <- function(icon, value) {
  ...
  #res <- tclvalue(.Tcl(paste("catch {winico text ", icon, " {", value, "}}", sep = "")))
  #if (res != "0")
  #  stop("Error while changing text of the icon!")
  #icon
}
